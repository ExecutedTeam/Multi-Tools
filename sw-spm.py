...
def ytmp4():
    try:
        link=input(f"   {putih}Link {R}:{Y} ")
        yt=YouTube(link)
        print(f"   {Y}#{putih} Title {R}: {H}",yt.title)
        print(f"   {Y}#{putih} Views {R}: {H}",yt.views)
        print(f"   {Y}#{putih} Publish On {R}: {H}",yt.publish_date)
        dl=yt.streams.get_highest_resolution()
        mp4dl=dl.download()
        print(f"   {Y}#{putih} Result --{U}>{H} Success Download "+yt.title+".mp4")
    except pytube.exceptions.RegexMatchError:
        sys.exit(f"   {putih}Result {R}:{H} Link Not Valid {R}!!!{putih}")

def ytmp3():
    try:
        link=YouTube(str(input(f"   {putih}Link {R}:{Y} ")))
        sound=link.streams.filter(only_audio=True).first()
        out_file=sound.download(output_path="")
        base,ext=os.path.splitext(out_file)
        new_file=base+'.mp3'
        os.rename(out_file, new_file)
        print(f"   {Y}#{putih} Title {R}: {H}",link.title)
        print(f"   {Y}#{putih} Views {R}: {H}",link.views)
        print(f"   {Y}#{putih} Publish On {R}: {H}",link.publish_date)
        print(f"   {Y}#{putih} Result --{U}>{H} Success Download "+new_file)
    except pytube.exceptions.RegexMatchError:
        sys.exit(f"   {putih}Result {R}:{H} Link Not Valid {R}!!!{putih}")

baca3 = requests.get("https://sfile.mobi/1Jupdtp276e7",headers={"User-Agent":"Mozilla/5.0 (Linux; Android 11; CPH2325) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36"}).text
...
{U}│  {Y}•{H} 03{R}.{putih} Spam SWC {R}({putih}Sms{R},{putih}Wa{R},{putih}Call{R})     {Y}•{H} 04{R}.{putih} Support Admin                         {U}      │
{U}│  {Y}•{H} 05{R}.{putih} About Tools                {Y}•{H} 06{R}.{putih} Ytmp4 DL                                    {U}│
{U}│  {Y}•{H} 07{R}.{putih} Ytmp3 DL                   {Y}•{H} 08{R}.{putih} Exit Tools                                  {U}│
{U}└───┬─────────────────────────────────────────────────────────────────────────────────┘""")
...
# Copyright By AmmarrBN & SanzzXD
# Made With Executed Team & Sanzz Team ❤️